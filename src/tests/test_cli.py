...
class MockAppAllFeatures(AgentApp):
    ...
    def get_mcps(
        ...
        return [
            MCPConfig(
                path=Path("~/.config/allfeatures/mcp.json"),
                type="global",
                servers=servers,
            )
        ]
...
class MockAppNoGlobalRules(AgentApp):
    ...
    def get_mcps(
        ...
        return [
            MCPConfig(
                path=Path("~/.config/noglobal/mcp.json"), type="global", servers=servers
            )
        ]
...
class MockAppNoWorkspaceRules(AgentApp):
    ...
    def get_mcps(
        ...
        return [
            MCPConfig(
                path=Path("~/.config/nows/mcp.json"), type="global", servers=servers
            )
        ]
...
def runner() -> CliRunner:
    ...
    project_root = Path(
        __file__
    ).parent.parent.parent  # Assuming tests are in src/tests/
...
def test_agapps_list(runner: CliRunner):
    ...
    assert "AllFeatures App" in result.output
    assert (
        "Yes" in result.output
    )  # For MCPs, Global, Workspace - a bit simplistic but checks presence
    ...
    assert any(
        "NoWorkspaceRules App" in line and "Yes" in line and "No" in line
        for line in lines
    )

    # Check for NoFeatures App: No | No | No
    assert any(
        "NoFeatures App" in line
        and "No" in line
        and "No" in line
        and line.count("No") >= 3
        for line in lines
...
def test_agapps_view_workspace_path_invalid_non_existent(runner: CliRunner):
    ...
    assert (
        "Error: Invalid value for 'WORKSPACE_PATH': Directory '/path/to/non_existent_dir' does not exist."
        in result.output
    )
...
def test_agapps_view_workspace_path_invalid_is_file(runner: CliRunner, tmp_path: Path):
    ...
    assert result.exit_code != 0  # Click usually exits with 2 for bad param
    assert (
        f"Error: Invalid value for 'WORKSPACE_PATH': Directory '{str(file_path)}' is a file."
        in result.output
    )
...
def test_agapps_mcps_details(runner: CliRunner):
    ...
    assert (
        "KEY = VALUE" in result.output
    )  # Corrected assertion for non-redacted short value
    assert (
        "NoMCPs App" not in result.output
    )  # Should not show details for apps with no MCPs
...
def test_agapps_rules_global_only(runner: CliRunner):
    ...
    assert (
        "NoGlobalRules App Rules:" not in result.output
    )  # Explicitly check it's NOT there
...
def test_agapps_rules_with_workspace(runner: CliRunner, tmp_path: Path):
    ...
    result_after_nomcps_ws_created = runner.invoke(
        cli, ["rules", str(tmp_path)]
    )  # Re-invoke if needed, or create before.
...
def test_agapps_rules_no_rules_app(runner: CliRunner, tmp_path: Path):
    ...
    result_workspace = runner.invoke(
        cli, ["rules", str(tmp_path), "--app", "no-features"]
    )
    ...
    assert (
        Path(tmp_path).name in result_workspace.output
    )  # ensure context is mentioned (directory name)
...
def test_agapps_rules_invalid_workspace_path(runner: CliRunner):
    ...
    assert (
        "Error: Invalid value for '[WORKSPACE_PATH]': Directory '/path/to/non_existent_dir_for_rules' does not exist."
        in result.output
    )
