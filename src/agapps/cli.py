...
def mask_sensitive_value(value: str) -> str:
    ...
    non_sensitive_values = [
        "true", "false", "yes", "no", "medium", "high", "low",
        "claude", "sonar", "gpt", "debug", "production", "development"
    ]
...
def _display_mcp_summary_table(
    ...
    for i in range(0, len(all_mcp_names_summary), chunk_size):
        ...
        for app_name_key, app_class in sorted_apps_list:
            ...
            mcps_by_name = {mcp.name: mcp for mcp in app_mcps_data.get(app_instance.name, [])}
    ...
    if details:        
        ...
        for app_name_detail, app_class_detail in target_apps_to_display.items():
            ...
            if mcps_list_detail:
                ...
                config_paths_detail = gathered_app_mcp_config_paths.get(app_instance_detail.name, [])
                if config_paths_detail:
                    home_dir_str = str(Path.home())
                    formatted_paths = [
                        ("~" + str(p)[len(home_dir_str):] if str(p).startswith(home_dir_str) else str(p)).replace(" ", "\\ ")
                ...
                for mcp_detail in mcps_list_detail:
                    console_obj.print(f"  • [blue]{mcp_detail.name}[/blue]: {mcp_detail.command}")
                    if mcp_detail.envs:
    ...
    for app_name_iter, app_class_iter in target_apps_to_display.items():
        ...
        if workspace_path_str:
            if workspace_rules_list:
                console_obj.print(f"    Workspace rules for [green]{workspace_path_str}[/green]:")
                for rule in workspace_rules_list:
                    print_rule_info(rule)
            else:
                console_obj.print(f"    No workspace rules found in [green]{workspace_path_str}[/green].")
...
@cli.command("view")
@click.argument("workspace_path", required=True, type=click.Path(exists=True, file_okay=False, dir_okay=True, readable=True))
...
def view_workspace_info(workspace_path: str, app_filter: Optional[str]) -> None:
    ...
    console.print(Panel.fit(f"Information for Workspace: [green]{workspace_path}[/green]", style="bold"))
    if app_filter:
